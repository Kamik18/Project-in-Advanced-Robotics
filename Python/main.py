...
if __name__ == "__main__":
    ...
    admittance_control: AdmittanceControl = AdmittanceControl(
        Kp=10, Kd=20, tr=0.1, sample_time=TIME)
    ...
    while True:
        ...
        #print(f"Admittance control: {force_q[0:3]}")
        _, p, dp, ddp = admittance_control.Translation(
           wrench=force_q[0:3], p_ref=[0, 0, 0])
        
        print(f"Admittance control: {dp}")
      
        # Set the translational velocity of the robot
        rtde_c.speedL([dp[0][0], dp[1][0], dp[2][0], 0, 0, 0], ACCELERATION, TIME)
