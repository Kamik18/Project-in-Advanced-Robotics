...
class Demo:
    ...
    def __init__(self) -> None:
        ...
        self.model = mujoco.MjModel.from_xml_path("./Example_exercise/franka_emika_panda/scene_pick.xml")
    ...
    def control(self, xpos_d, xquat_d):
        xpos = self.data.body("hand").xpos
        xquat = self.data.body("hand").xquat
        ...
        v = J @ self.data.qvel
        for i in range(1, 8):
            dofadr = self.model.joint(f"joint{i}").dofadr
            self.data.actuator(f"actuator{i}").ctrl = self.data.joint(
                f"joint{i}"
            ).qfrc_bias
            self.data.actuator(f"actuator{i}").ctrl += (
    ...
    def step(self) -> None:
        xpos0 = self.data.body("hand").xpos.copy()
        xpos_d = xpos0
        xquat0 = self.data.body("hand").xquat.copy()
